...
describe('lib/http-proxy.js', function() {
  describe('HTTP2 #createProxyServer', function() {
    ...
    describe('HTTP2 to HTTP', function () {
      it('should proxy the request en send back the response', function (done) {
        ...
        http2.get({
          ...
          expect(res.httpVersion).to.eql(2.0);
    ...
    describe('HTTP to HTTP2', function () {
      it('should proxy the request en send back the response', function (done) {
        var ports = { source: gen.port, proxy: gen.port };
        var source = http2.createServer({
          key: fs.readFileSync(path.join(__dirname, 'fixtures', 'agent2-key.pem')),
          cert: fs.readFileSync(path.join(__dirname, 'fixtures', 'agent2-cert.pem')),
          ciphers: 'AES128-GCM-SHA256',
        }, function (req, res) {
    ...
    describe('HTTP2 to HTTP2', function () {
      it('should proxy the request en send back the response', function (done) {
        var ports = { source: gen.port, proxy: gen.port };
        var source = http2.createServer({
          key: fs.readFileSync(path.join(__dirname, 'fixtures', 'agent2-key.pem')),
    ...
    describe('HTTP2 not allow SSL self signed', function () {
    ...
    describe('HTTP2 to HTTP using own server', function () {
      it('should proxy the request en send back the response', function (done) {
        var ports = { source: gen.port, proxy: gen.port };
        var source = http.createServer(function(req, res) {
          expect(req.method).to.eql('GET');
        ...
        var ownServer = http2.createServer({
          key: fs.readFileSync(path.join(__dirname, 'fixtures', 'agent2-key.pem')),
          cert: fs.readFileSync(path.join(__dirname, 'fixtures', 'agent2-cert.pem')),
          ciphers: 'AES128-GCM-SHA256',
        }, function (req, res) {
