...
RoutingProxy.prototype._getKey = function (options) {
  if (!options || ((!options.host || !options.port)
    && (!options.target || ((!options.target.host || !options.target.port) || !options.target.socketPath))
    && !options.socketPath)) {
    throw new Error('options.host and options.port or options.socketPath or options.target are required.');
  }

  return (options.socketPath || (options.target && options.target.socketPath)) ? (options.socketPath || options.target.socketPath) : [
