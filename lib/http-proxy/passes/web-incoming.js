	...
	function stream(req, res, options, _, server, clb) {
		...
		var _start = function (body) {
			if (options.forward) {
				...
				body && forwardReq.write(body);
				if (!options.target) {
					return res.end();
				}
			}
			// Request initalization
			proxyReq = (options.target.protocol === 'https:' ? https : http).request(
				common.setupOutgoing(options.ssl || {}, options, req)
				);
			//console.log('proxyReq.write', body);
		...
		// And we begin!
		server.emit('start', req, res, options.target, _start);
